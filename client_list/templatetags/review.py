...
from farproof.client_list.models import Version, Page, Revision, Comment
...
def comment_add(request, version, item, job, client):
	...
	if request.method == 'POST' and int(request.POST['from_page']) == version.rel_num:
		...
		for p in pages:
			...
			print(str(c)+'\t\t'+str(current_version.abs_num)+'\t\t'+str(current_version.rel_num)+'\t\t'+str(current_rev.rev_number)+'\t\t'+str(revision.rev_number)+'\t\t'+str(changes))
